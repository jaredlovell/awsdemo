...
def lambda_handler(event, context):
    ...
    for region in regions:
        ...
        result = ec2.describe_snapshots(Filters=[{'Name':'tag:Origin', 'Values':['automated_backup',]},])
        keep_count=0
        del_count=0
        for snapshot in result['Snapshots']:
            ...
            if (now - snapshot_time) > timedelta(retention_days):
                ...
                try: delete_snapshot(snapshot['SnapshotId'], reg)
                except: print("failed to delete: " + str(snapshot['SnapshotId']))
                else: del_count+=1
            else:
                print("Snapshot is newer than configured retention of %d days so we keep it" % (retention_days))
                keep_count+=1
        print("checked: "+str(len(result['Snapshots']))+ ' deleted: '+ str(del_count) + " snapshots.")